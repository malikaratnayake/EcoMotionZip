...
class Writer(LoggingThread):
    """
    Unified writer that prefers OpenCV's VideoWriter, and falls back to an FFmpeg pipe
    for codecs OpenCV doesn't typically support (e.g., H.265/HEVC).

    - Logs the chosen backend (opencv/ffmpeg) and the codec/encoder used.
    - Preserves CSV sidecar and optional frame saving / timestamp overlay behavior.
    """
    def __init__(
        self,
        writing_queue: Queue,
        ...
        self.flush_thresh = int(0.25 * writing_queue.maxsize) if writing_queue.maxsize > 0 else 1
        self.info(f"Will flush buffer to output file every {self.flush_thresh} frames")
        ...
        if self.backend == "opencv":
            # If codec string is empty or odd-length, default to mp4v to avoid exceptions
        ...
        self._ffmpeg_encoder_name: Optional[str] = None  # purely for logging

    @classmethod
    def from_reader(
        cls,
        reader: Any,  # Reader-like with get_fps()/get_frame_size()
        writing_queue: Queue,
        filepath: str,
        stop_signal: Event,
        logger: logging.Logger,
    ...
    def _select_backend(self, codec: str) -> str:
        """
        Decide whether to use OpenCV or FFmpeg.
        Default: OpenCV. Fallback: FFmpeg for known-unsupported codecs (e.g., HEVC).
        """
        v = (codec or "").upper()
        hevc_names = {"H265", "HEVC", "HVC1", "HEV1"}
        if v in hevc_names:
            return "ffmpeg"
        # Otherwise default to OpenCV
        return "opencv"
    ...
    def _map_codec_to_ffmpeg(self, ext: str) -> List[str]:
        """
        Translate the user-provided `video_codec` into an FFmpeg encoder and tags.
        Also stores encoder name for logging.
        """
        tag_args: List[str] = []
    ...
    def _close_ffmpeg(self) -> None:
        ...
        try:
            ...
            if rc != 0:
                self.warning(f"FFmpeg exited with code {rc}. Output may be incomplete: {self.filepath}")
    ...
    def run(self) -> None:
        """
        Main writing loop shared by both backends.
        """
        # Initialize backend
        vw = None
        if self.backend == "opencv":
            ...
            if not vw.isOpened():
                # If OpenCV fails to open (e.g., build lacks codec), fall back to FFmpeg
        ...
        while loop_is_running:
            ...
            self.debug(
                f"Queue size exceeded ({self.writing_queue.qsize() >= self.flush_thresh}) "
                f"OR stop signal ({self.stop_signal.is_set()})"
            )
            ...
            for _ in range(frames_to_flush):
                try:
                    ...
                    self.warning("Waited too long for frame! Exiting...")
                    loop_is_running = False
                    break
